	...
	public class AccessTokenService : IAccessTokenService
	{
		...
		public async Task<string> GetClientAccessTokenAsync(String scope)
		{
			...
			catch (System.Exception ex)
			{
				...
				throw new Exception.TerraUnderpinningException(this._errors.UnderpinningService.Code, this._errors.UnderpinningService.Message, (int?)response?.StatusCode, UnderpinningServiceType.DataManagement, this._logCorrelationScope.CorrelationId, includeErrorPayload ? errorPayload : null);
			}
		...
		public async Task<string> GetExchangeAccessTokenAsync(String requestAccessToken, String scope)
		{
			...
			if (token == null) return null;
			if(token.ExpiresIn <=0) throw new Exception.TerraUnauthorizedException(this._errors.UnauthorizedToken.Code, this._errors.UnauthorizedToken.Message);
		...
		{
			return $"{nameof(Terra.AiModelRegistry)}:{nameof(AccessTokenService)}:{this._config.ClientId}:no-exchange:{key ?? String.Empty}:v0";
		}
		private String CacheKeyExchange(String key)
		{
			return $"{nameof(Terra.AiModelRegistry)}:{nameof(AccessTokenService)}:{this._config.ClientId}:exchange:{key ?? String.Empty}:v0";
		}
	}
}
